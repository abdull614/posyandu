...
import 'package:myapp/model/ibu_hamil_model.dart'; // Pastikan file ini ada
import 'package:myapp/service/imunisasi_service.dart'; // Pastikan file ini ada

class JadwalIbuHamil extends StatefulWidget {
  @override
  _JadwalIbuHamilState createState() => _JadwalIbuHamilState();
}

class _JadwalIbuHamilState extends State<JadwalIbuHamil> {
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate:
          DateTime.now(), // Tanggal pertama yang dapat dipilih adalah hari ini
      lastDate:
          DateTime(2100), // Tanggal terakhir yang dapat dipilih (misalnya 2100)
    );
    if (picked != null) {
      setState(() {
  ...
  void _validateAndSubmit() async {
    try {
      if (_dateController.text.isNotEmpty) {
        ...
        // Membuat objek IbuHamil
        IbuHamil ibuHamil = IbuHamil(
        ...
        await _imunisasiService.addIbuHamil(ibuHamil);

        // Siapkan pesan notifikasi
        String notifMessage = 'Pendaftaran imunisasi berhasil disimpan:\n'
            'Nama: ${ibuHamil.nama}\n'
            'Alamat: ${ibuHamil.alamat}\n'
            'Tanggal Imunisasi: ${_dateFormat.format(ibuHamil.tanggalImunisasi)}';
        ...
        Navigator.pushReplacement(
          ...
          MaterialPageRoute(
            builder: (context) =>
                NotifScreen(notificationMessage: notifMessage),
          ),
        );
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Jadwal Ibu Hamil'),
