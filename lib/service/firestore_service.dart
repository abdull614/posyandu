...
class FirestoreService {
 ...
 Future<void> tambah({required String collectionPath, required
Map<String, dynamic> item}) async {
 await _firestore.collection(collectionPath).add(item);
 }
 /// Mengupdate item di koleksi tertentu berdasarkan ID
 Future<void> update({required String collectionPath, required String id,
required Map<String, dynamic> item}) async {
 ...
 Future<void> hapus({required String collectionPath, required String id})
async {
 ...
 return
_firestore.collection(collectionPath).snapshots().map((snapshot) =>
...
_firestore.collection(collectionPath).doc(id).get();
 return fromMap(doc);
 }
}